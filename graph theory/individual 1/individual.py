...
def main_individual() -> str:
    ...
    count_vertices: int = int(input("Введите кол-во вершин: "))  # Вводим кол-во вершин

    # Вводим сами степени вершин
    vertices_degree: list = [int(input(f"Введите вершину: {i + 1} - ")) for i in range(count_vertices)]
    ...
    sort_vertices: list = [i for i in vertices_degree if i != 0]  # Удаляем нулевые вершины графа
    # Именуем каждую вершину графа, 0 вершины не учитываются
    dict_vertices: list = [list(j) for j in list({i: sort_vertices[i] for i in range(len(sort_vertices))}.items())]
    result_vertices: list = list()  # Список связей вершин графа
    if not sort_vertices:  # Проверяем не состоит ли граф только из нулевых вершин
        return "Граф не связанный"

    sum_vertices: int = sum(sort_vertices)  # Вычисляем сумму степенней вершин
    vertices_graph: bool = algorithm_hackimi(dict_vertices, result_vertices)  # Строим граф по алгоритму Хавел-Хакими
    # Проверяем существования полного графа
    full_graph: bool = ((count_vertices * (count_vertices - 1)) / 2) * 2 == sum_vertices
    # Проверяем существования простого графа
    simple_graph: bool = (count_vertices <= sum_vertices and sum_vertices % 2 == 0 and vertices_graph)
